    ...
    <div className="bg-white rounded-[2rem] border border-slate-100 overflow-hidden hover:shadow-lg hover:shadow-slate-200/50 transition-all duration-300">
      <div className="px-8 py-4 bg-slate-50/50 border-b border-slate-100 flex justify-between items-center">
        ...
        <div className="flex items-center gap-4">
          <button
            onClick={onEdit}
            className="flex items-center gap-1.5 text-brand-blue hover:text-brand-blueDark text-[10px] font-black uppercase tracking-widest transition-colors"
          >
            <Edit3 size={13} /> Edit
          </button>
          <button
            onClick={() => setExpanded((v) => !v)}
            className="flex items-center gap-1 text-slate-400 hover:text-brand-blue text-[10px] font-black uppercase tracking-widest transition-colors"
          >
...
export default function Step6() {
  ...
  return (
    <div className="space-y-6">
      {/* Personal */}
      <ReviewCard title="Personal Information" step={1} onEdit={() => setStep(1)}>
      ...
      {/* Family */}
      <ReviewCard title="Family Background" step={2} onEdit={() => setStep(2)}>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
      ...
      {/* Education */}
      <ReviewCard title="Education Background" step={3} onEdit={() => setStep(3)}>
        <div className="space-y-6">
      ...
      {/* Academics */}
      <ReviewCard title="Academics" step={4} onEdit={() => setStep(4)}>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
      ...
      {/* Payment */}
      <ReviewCard title="Payment Details" step={5} onEdit={() => setStep(5)}>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
